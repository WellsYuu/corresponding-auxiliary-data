...
public class SnakeGameServer {
    ...
    public SnakeGameServer(int port) {
        this.port = port;
        gameEngine = new SnakeGameEngine(60, 60, 700);
    ...
    public void run() throws Exception {
        ...
        gameEngine.setListener(new SnakeGameEngine.SnakeGameListener() {
            ...
            //by pz 聊天信息发送
            @Override
            public void noticeChatMessage(ChatMessage message){
    ...
    private void sendChatMessage(ChatMessage message){
        String prefix = "msg\r\n";
        for (Channel channel : channels){
            channel.writeAndFlush( new TextWebSocketFrame(prefix+JSON.toJSONString(message)));
        }
    }
