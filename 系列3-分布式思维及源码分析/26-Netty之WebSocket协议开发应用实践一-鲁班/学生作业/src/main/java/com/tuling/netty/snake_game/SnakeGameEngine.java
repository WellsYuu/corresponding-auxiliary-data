...
public class SnakeGameEngine {
    ...
    //by pz 触发接收聊天消息
    public void doChat(String accountId,String message) {
        if (listener != null) {
            ChatMessage chatMessage = new ChatMessage();
            chatMessage.setMsg(message.substring(5));
            //昵称取服务器的，也可以从客户端传
            for (SnakeEntity snakeEntity : snakes.values()) {
                if (snakeEntity.getAccountId().equals(accountId)) {
                    chatMessage.setName(snakeEntity.getGameName());
                    break;
                }
            }
            listener.noticeChatMessage(chatMessage);
        }
    }
...
//                    snake.moveStep();
        ...
        for (SnakeEntity snake : snakes.values()) {
           ...
           boolean died = false;
            ...
            if (died) {
                //如果蛇死亡，将蛇身变成食物
                ArrayList<Integer[]> bodyNodes = snake.getBodys();
                int i = 0;

                for (Integer[] node : bodyNodes) {
                    int index = node[0] * mapWidth + node[1];
                    if (index < 0 || index >= mapsMarks.length) {
                        continue;
                    }
                    boolean odd = i % 2==0;
                    getMark(index).footNode = odd  ? 1 : 0;
                    getMark(index).snakeNodes = 0;
                    changeNodes.add(node);
                    if (odd) {
                        foods.add(new Food(node, 1));
                    }
                    i++;
                }
            }
...
//            logger.warn("找不到指定帐户");
    ...
    public static interface SnakeGameListener {
         ...
         * @param message 聊天信息
         * */
        void noticeChatMessage(ChatMessage message);
    }
}
