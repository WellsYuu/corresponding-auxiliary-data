...
public abstract class ClassUtils {
    ...
    public static boolean matchesTypeName(Class<?> clazz, String typeName) {
        return (typeName != null && (typeName.equals(clazz.getName()) || typeName.equals(clazz.getSimpleName()) || (clazz
                .isArray() && typeName.equals(getQualifiedNameForArray(clazz)))));
    }
     ...
     *            in (may be {@code null} when accepting all declared
     *            interfaces)
     * @return all interfaces that the given object implements as array
     */
    ...
    public static Set<String> getClassesByPackageName(String packageName, String packageNameNeedExclude){
        Set<String> classNames = new HashSet<>();
        ClassLoader loader = Thread.currentThread().getContextClassLoader();
        
        for(String tmpPackageName : packageName.split(";")){
            String packagePath = tmpPackageName.replace(".", "/");
     
            URL url = loader.getResource(packagePath);
            if (url != null) {
                String protocol = url.getProtocol();
                if (protocol.equals("file")) {
                    classNames.addAll(getClassNameFromDir(url.getPath(), packageName, true));
                } else if (protocol.equals("jar")) {
                    try {
                        classNames.addAll(getClassNameFromJar(url, packageName, packageNameNeedExclude));
                    } catch (IOException e) {
                        e.printStackTrace();
                    };
                }
    ...
    public static Set<String> getClassNameFromJar(URL url, String packageName, String packageNameNeedExclude) throws IOException {  
        Set<String> classNames = new HashSet<String>();
        ...
        while (jarEntries.hasMoreElements()) {  
            ...
            if(clazzName.startsWith(packageNameNeedExclude)) continue;
            
            String prefix = clazzName.substring(0, clazzName.lastIndexOf("."));  
            if (prefix != null) {
                if(prefix.startsWith(packageName)){  
    ...
    public static String getAppDetail(){
        /*String classBasePath = Thread.currentThread().getContextClassLoader().getResource("/").getPath();
        if (classBasePath.indexOf("/WEB-INF") != -1) {
            String tmpPath = classBasePath.substring(0, classBasePath.lastIndexOf("/WEB-INF"));
